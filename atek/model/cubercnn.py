...
def create_cubercnn_model(model_config: Dict) -> torch.nn.Module:
    """
    Build CubeRCNN model from config
    """
    # build model and load weights
    model = build_model(model_config["cubercnn_cfg"], priors=None)
    _ = DetectionCheckpointer(model, save_dir=model_config["ckpt_dir"]).resume_or_load(
        model_config["cubercnn_cfg"].MODEL.WEIGHTS, resume=True
    )

    return model


class CubercnnPredictionConverter(BaseCallback):
...
def save_predicted_canonical_object_poses_to_csv(
    ...
    if saver_config["bbox3d_csv"] is None:
        return

    selected_prototypes = read_txt(saver_config["prototype_file"])
    prototypes = [instance_metadata[str(uid)]["prototype_name"] for uid in object_uids]
    bbox3d_data = load_bbox3d_data(saver_config["bbox3d_csv"])
...
class CubercnnPredictionSaver(BaseCallback):
    def __call__(self, prediction_list):
        ...
        instance_metadata = read_json(self.config["metadata_file"])
        inst_ids_to_cat_ids = read_json(self.config["category_id_remapping_json"])
        cat_ids_to_inst_ids = {
