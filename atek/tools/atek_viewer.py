...
from atek.data_preprocess.frameset_group_generator import (
...
TRAJECTORY_COLOR: List[int] = [30, 100, 30]
EVAL_CAMERA_NAME: str = "camera-rgb"
GT_COLOR: List[int] = [30, 200, 30]
...
def log_static_data(frameset_group_generator):
    ...
    frameset_trajectory = [
        (
            frameset_aligner.get_T_world_frameset_by_index(index) @ T_frameset_device
        ).translation()[0]
...
def log_instance_data(
    ...
    rr.log(
        ...
        rr.Boxes3D(
            sizes=bb3d_sizes_infer,
            centers=bb3d_centers_infer,
            radii=0.01,
            colors=PRED_COLOR,
            labels=labels_infer,
        ),
...
def log_frameset_group_data(frameset_group, eval_frame_data, visualize_label):
        ...
        for frame in frameset.frames:
            ...
            if frame.bb2ds is not []:
                ...
                if bb2ds.shape[1] != 4:
                    ...
                    continue
                bb2d_XYXY = np.array(
                    [bb2ds[:, 0], bb2ds[:, 2], bb2ds[:, 1], bb2ds[:, 3]]
                ).transpose()
            ...
            if eval_frame_data is not [] and frame_id in eval_frame_data.keys():
                ...
                for eval_frame in eval_frame_data[frame_id]:
                    ...
                    T_camera_obj_mat[0:3, 0:3] = np.array(
                        [list for list in eval_frame["R_cam_obj"]]
                    ).reshape(3, 3)
...
def setup_atek(adt_path: str):
    ...
    rgb_adt_gt_data_processor = AdtGtDataProcessor(
        "adt_gt", "ADT", StreamId("214-1"), data_paths
    )
    slaml_adt_gt_data_processor = AdtGtDataProcessor(
        "adt_gt", "ADT", StreamId("1201-1"), data_paths
    )
    slamr_adt_gt_data_processor = AdtGtDataProcessor(
        "adt_gt", "ADT", StreamId("1201-2"), data_paths
    )
    ...
    frame_data_processors = [
        rgb_data_processor,
        slaml_data_processor,
        slamr_data_processor,
    ]
    ...
    frameset_group_generator = FramesetGroupGenerator(
        frameset_aligner=frameset_aligner,
...
def log_infer_data(data_path):
    if os.path.exists(data_path) is False:
        print(f"Cannot find inference data {data_path}")
        return []
...
def run_viewer(frameset_group_generator, eval_frame_data, visualize_label: bool):
    ...
    log_static_data(frameset_group_generator)
    ...
    for index in range(0, num_frameset_groups):
        log_frameset_group_data(
            frameset_group_generator.get_frameset_group_by_index(index),
            eval_frame_data,
            visualize_label,
        )
    ...
    for obj in dataset:
        ...
        for camera_name, streamid in zip(wds_camera_names, wds_camera_streamids):
            ...
            for frameset_index in range(num_framesets):
                ...
                category_id_to_name = obj[f"F#{streamid}+category_id_to_name"][
                    frameset_index
                ]
                ...
                bb3d_sizes = []
                bb3d_centers = []
                object_dimensions = obj[f"F#{streamid}+object_dimensions"][
                    frameset_index
                ]
...
def main():
    ...
    parser.add_argument(
        "--label",
        action="store_true",
        help="Visualize bounding box labels in 3D viewer",
    )
    parser.add_argument(
        "--web-port",
        default=8888,
    ...
    if args.wds is not None:
        ...
        print(
            f"ATEK viewer set up with adt input {args.adt_path} and inference results {args.infer}"
        )
        frameset_group_generator = setup_atek(args.adt_path)
        eval_frame_data = log_infer_data(args.infer)
